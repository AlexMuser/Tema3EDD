...
public class Principal
{
    ...
    public Pila infPso(String expr)
    {
        ...
        for (int i = 0; i < expr.length(); i++)
        {
            ...
            if (expr.charAt(i) == '$') //3
            {
                if (op.empty()==false && (char)op.stacktop() == '$')
                {
                    var.push(op.pop());
                }
                op.push(expr.charAt(i));
            ...
            {
                while (op.empty() == false && ((char)op.stacktop()=='$'||(char)op.stacktop()=='*'||(char)op.stacktop()=='/'))
                {
                    var.push(op.pop());
                }
                op.push(expr.charAt(i));
            ...
            {
                while (op.empty() == false && ((char)op.stacktop() == '$' || (char)op.stacktop() == '*' || (char)op.stacktop() == '/' || (char)op.stacktop() == '+' || (char)op.stacktop() == '-'))
                {
                    var.push(op.pop());
                }
                op.push(expr.charAt(i));
    ...
    public double evalCadPsa(Pila posf, ArrayList<Character> caract, ArrayList<Double> nums)
    {
        ...
        {
            if (Character.isLetter((char)posf.stacktop()))
            {
...
//                posf.pop();
//            }
            if (posf.empty()==false && (char)posf.stacktop()=='*')
            {
                double x1 = (double)evalua.pop(), x2 = (double)evalua.pop();
                res= x2 * x1;
                evalua.push(res);
                posf.pop();
            }
